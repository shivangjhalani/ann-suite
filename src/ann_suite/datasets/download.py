...
def compute_ground_truth(
    ...
    for i, query in enumerate(queries):
        if metric in ("L2", "euclidean"):
            distances = np.linalg.norm(base - query, axis=1)
        elif metric in ("IP", "inner_product"):
            distances = -np.dot(base, query)
        elif metric in ("cosine", "angular"):
            # Normalize and use dot product
            query_norm = query / (np.linalg.norm(query) + 1e-10)
            base_norms = base / (np.linalg.norm(base, axis=1, keepdims=True) + 1e-10)
            distances = -np.dot(base_norms, query_norm)
        else:
            distances = np.linalg.norm(base - query, axis=1)
...
def download_dataset(
    ...
    if name in datasets:
        ...
        for ds_name, ds_config in datasets.items():
            if "subsets" in ds_config and name in ds_config["subsets"]:
                parent_config = ds_config
                subset_config = ds_config["subsets"][name]
        ...
        if not found:
            ...
            for ds_name, ds_config in datasets.items():
                ...
                if "subsets" in ds_config:
                    available.extend(ds_config["subsets"].keys())
    ...
    download_file(parent_config["url"], hdf5_path, quiet=quiet)
